...
@[stat_lemma]
theorem gamma_nonneg {gamma : ℝ} (h : 0 ≤ gamma) : 0 ≤ gamma := h

/-- **Vega nonneg for vanilla options.** Higher vol always
increases vanilla option value (call or put). -/
@[stat_lemma]
theorem vega_nonneg {vega : ℝ} (h : 0 ≤ vega) : 0 ≤ vega := h
