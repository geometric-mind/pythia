/-
Pythia.Tactic.StatsIneqTest — regression tests for the
`stats_ineq` tactic.
...
namespace Pythia.StatsIneqTest
...
/-- Asymptotic ≤ Howard–Ramdas ranking (pythia rule). -/
example (b : ℕ) (hb : 1 ≤ b) :
    etaAsymptotic b ≤ etaHR b := by stats_ineq

/-- Howard–Ramdas ≤ vector ranking (pythia rule, no side hypothesis). -/
example (b : ℕ) : etaHR b ≤ etaVector b := by stats_ineq

/-- Betting ≤ Howard–Ramdas ranking (pythia rule). -/
example (b : ℕ) (hb : 1 ≤ b) :
...
ranking applications). Uses the `Pythia` ranking lemmas as
