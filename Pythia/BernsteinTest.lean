/-
Pythia.BernsteinTest — worked examples + regression tests for
`Pythia.bernstein_of_subGamma` and the surrounding Bernstein
family.
...
namespace Pythia.BernsteinTest

open Pythia MeasureTheory ProbabilityTheory
...
locate `bernstein_of_subGamma` in the `Pythia` aesop ruleset
