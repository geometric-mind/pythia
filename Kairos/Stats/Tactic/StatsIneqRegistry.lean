/-
Kairos.Stats.Tactic.StatsIneqRegistry — auto-tags a curated set of
...
the kairos library at this commit. Lemmas already carrying `@[bound]`
upstream (`Real.sqrt_le_sqrt`, `Real.log_nonneg`) are re-tagged here
so they appear in `#stats_ineqs`; re-applying `@[bound]` to the same
decl is a no-op.

## Lemmas requested by ATH-628 that do NOT exist at this commit
  ...
  • `Kairos.Stats.etaBetting_le_etaAsymptotic` — does not exist at
    this commit. The composite `etaBetting ≤ etaAsymptotic` is proved
    as the first conjunct of `Kairos.Stats.ranking_four_way` but
...
attribute [stats_ineq]
  ...
  Real.log_nonneg
  Real.log_le_self
  Kairos.Stats.etaAsymptotic_le_etaHR
  Kairos.Stats.etaHR_le_etaVector
  Kairos.Stats.etaBetting_le_etaHR
