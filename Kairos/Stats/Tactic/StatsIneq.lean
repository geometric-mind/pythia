/-
Kairos.Stats.Tactic.StatsIneq — the `stats_ineq` tactic.

A domain inequality hammer for the kairos-stats library, modeled on
...
* `stats_ineq` tactic. Tries (in order):
  1. `bound` — the Mathlib hammer with the kairos-tagged rule set.
...
ATH-628. Pythia headline tactic shipped in `Tactic/Pythia.lean`; this
...
/-- `@[stats_ineq]` — register a theorem as a kairos-stats inequality
...
initialize registerBuiltinAttribute {
  name := `stats_ineq
  descr := "Register theorem as a kairos-stats `@[bound]` rule for the `stats_ineq` tactic."
...
/-- `stats_ineq` — kairos-stats inequality hammer.
