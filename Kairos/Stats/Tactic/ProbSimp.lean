/-
Kairos.Stats.Tactic.ProbSimp — the `pdf_simp` / `prob_simp` tactic and
the `@[prob_simp]` curated simp-set for probability normalization.

A domain `simp`-based normalizer for the kairos-stats library, modeled
...
ATH-630. Companion to `pythia` (ATH-608) and `stats_ineq` (ATH-628).
-/
import Mathlib
...
/-- `@[prob_simp]` — register a theorem as a kairos-stats probability
...
initialize registerBuiltinAttribute {
  name := `prob_simp
  descr := "Register theorem as a kairos-stats `@[simp]` rule for the `prob_simp` / `pdf_simp` tactic."
...
/-- `prob_simp` — kairos-stats probability normalization tactic.
