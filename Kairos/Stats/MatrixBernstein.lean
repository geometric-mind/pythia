/-
Kairos.Stats.MatrixBernstein — Tropp's matrix Bernstein inequality.
...
this module is excluded from `Kairos.Stats.AxiomAudit`. Closure of any
...
* `Kairos/Stats/Matrix/HermitianFunctionalCalculus.lean` — port the
  spectral-decomposition `f(A)` construction (item 3).
* `Kairos/Stats/Matrix/Klein.lean` — matrix Klein inequality (item 2).
* `Kairos/Stats/Matrix/Lieb.lean` — Lieb's concavity (item 1).
* `Kairos/Stats/Matrix/MGF.lean` — matrix moment-generating function
  + Lieb-Tropp master inequality + Tropp's matrix-CGF lemma (item 4).
* `Kairos/Stats/MatrixBernstein.lean` — close the sorries here.
...
the module docstring. Excluded from `Kairos.Stats.AxiomAudit` until
the proof lands. -/
